...
class DashboardController extends Controller
{
    ...
    public function saleRepDashboard()
    {
        ...
        $all_sales = Transaction::where('field_staff', $user->id)->where('created_at', 'LIKE', '%' . $date . '%')->select(\DB::raw('SUM(amount_due) as amount_due'))->first();
        ...
        $payments = Payment::where('received_by', $user->id)->where('payment_date', 'LIKE', '%' . $date . '%')->select(\DB::raw('SUM(amount) as total_amount'))->first();
        ...
        return response()->json(compact('user', 'customers', 'sales', 'debt', 'overdue', 'collections', 'today_visits', 'currency', 'month'), 200);
    }
